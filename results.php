...
foreach ($categories as $category => $candidates) {
    ...
    foreach ($candidates as $candidate) {
        // Get the vote count for each candidate using prepared statement
        $stmt = $conn->prepare("SELECT COUNT(*) as votes FROM votes WHERE category = ? AND candidate = ?");
        $stmt->bind_param("ss", $category, $candidate);
        $stmt->execute();
    ...
    foreach ($votes as $candidate => $vote_count) {
        echo htmlspecialchars($candidate) . ": " . htmlspecialchars($vote_count) . " votes<br>";
    }

    if (!empty($leading_candidates)) {
        $leading_names = array_map('htmlspecialchars', $leading_candidates);
        echo "<strong>Leading: " . implode(", ", $leading_names) . " with " . htmlspecialchars($max_votes) . " votes</strong><br>";
    }
}
